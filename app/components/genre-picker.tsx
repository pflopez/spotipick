...
export default function GenrePicker({genres, submitGenres}:{genres: string[], submitGenres: (genres: string[])=>void}){
    ...
    return (<section>
        ...
        {selectedGenres.map((d,i) => <div key={i}>s {d}</div>)}
        <div>
            {picks.map( (genre, i) => <GenreBtn key={i} genre={genre} onSelect={() => pickGenre(genre)}></GenreBtn>)}
        </div>
